import { RULES, bodyRadiusFor, radiusFor } from "../../web/src/shared/rules";
import { dist, wrapAngle } from "../../web/src/shared/geometry";
import { World, type Fly } from "../../web/src/shared/sim";
import { BOT_NAMES, hueForIndex } from "../../web/src/shared/names";
import { drawPot } from "./db";
import { config } from "./config";

/**
 * Flies the server plays itself so the first person in never lands in an
 * empty pit. They are real flies: they eat, they cut, they die and drop
 * what they carried. Their coins come from the pot and only from the pot —
 * when the pot is dry they spawn with nothing and are just bodies to avoid.
 * They never extract, so no pot coin ever leaves through a bot.
 */
export class Bots {
  ...
  /** Coins currently carried by bots — pot money, returned to the pot on a restart. */
  coins(): number {
    let sum = 0;
  ...
  /** Keeps `config.botCount` bots alive, one spawn every half second at most. */
  maintain(): void {
    if (this.ids.size >= config.botCount) return;
    if (this.world.time - this.lastSpawnAt < 0.5) return;
    this.lastSpawnAt = this.world.time;
    const coins = drawPot(RULES.botStakeCoins) ? RULES.botStakeCoins : 0;
  ...
  private decide(fly: Fly, now: number): void {
    ...
    if (fromCenter > RULES.arenaRadius * 0.82) {
      // 2. Too close to the wall: turn toward the centre, biased to the clearer side.
      const toCenter = Math.atan2(-fly.y, -fly.x);
      target = toCenter;
    } else if (ahead < 260) {
  ...
  /** Alive flies (players and bots) whose head is within `radius`, other than `fly`. */
  ...
  private clearance(fly: Fly, angle: number, others: Fly[], r: number): number {
    ...
    for (const o of others) {
      ...
      // half second: two flies aiming at one pellet must not meet halfway.
      const headRad = radiusFor(o.coins) + r + 30;
      const speed = o.boosting ? RULES.boostSpeed : RULES.baseSpeed;
      for (let k = 0; k <= 3; k++) {
        const ahead = speed * 0.7 * (k / 3);
        const hx = o.x + Math.cos(o.angle) * ahead;
        const hy = o.y + Math.sin(o.angle) * ahead;
